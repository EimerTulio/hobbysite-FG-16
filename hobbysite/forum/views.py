...
def list_view(request):
    """
    Displays a list of all threads, grouped by category 
    and separating the current user's articles from others.
    ...
    if request.user.is_authenticated:
        ...
        other_categories = ThreadCategory.objects.filter(
            post_category__in=other_threads).distinct()
    else:
...
def detail_view(request, pk):
    """
    Displays the details of a single thread including comments 
    and related threads of the same category.
    Handles comment submission for authenticated users
    Along with the HttpRequest object (request), 
    it takes in the primary key of the specific article to be displayed (pk)
    ...
    related_threads = Thread.objects.filter(
        category=threads.category).exclude(pk=threads.pk)
    profile = None
...
def update_view(request, pk):
    ...
    Along with the HttpRequest object (request), i
    t takes in the primary key of the specific thread to edit (pk).
