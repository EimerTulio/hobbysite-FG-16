...
def blog_list(request):
    ...
    if request.user.is_authenticated:
        user_articles = articles.filter(author__user=request.user)
        articles = articles.exclude(author__user=request.user)
    
    categories = ArticleCategory.objects.all().prefetch_related(Prefetch('article_set', queryset=articles))
