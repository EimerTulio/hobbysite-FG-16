    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if hasattr(request.user, 'profile'):
                form.add_error(
                    None, "A profile with this user already exists.")
...
def profile_update_view(request, username):
    ...
    if request.user != user:
        ...
        return redirect('user_management:forbidden')
